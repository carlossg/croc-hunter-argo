...
function moveBg() {
	...
	for (var i = 0;
		i < numSpawns;
		i++) { enemies[enemies.length] = new Enemy }
	...
	isPlaying = true;
	loop()
} function stopLoop() { isPlaying = false } function drawBg() {
	...
	for (var i = 0;
		i < 20;
		i++)this.bullets[this.bullets.length] = new Bullet(this);
	this.score = 0
...
Jet.prototype.checkDirection = function () { if (this.isUpKey && this.topY > 0) { this.drawY -= this.speed } if (this.isRightKey && this.rightX < gameWidth) { this.drawX += this.speed } if (this.isDownKey && this.bottomY < gameHeight) { this.drawY += this.speed } if (this.isLeftKey && this.leftX > 0) { this.drawX -= this.speed } };
...
function clearContextJet() { contextJet.clearRect(0, 0, gameWidth, gameHeight) } function Bullet(j) {
	this.srcX = 176;
	this.srcY = 501;
	this.drawX = -20;
	this.drawY = 0;
...
Bullet.prototype.recycle = function () { this.drawX = -20 };
...
Enemy.prototype.checkEscaped = function () { if (this.drawX + this.width <= 0) { this.recycleEnemy() } };
Enemy.prototype.recycleEnemy = function () {
	this.drawX = Math.floor(Math.random() * 1e3) + gameWidth;
	this.drawY = Math.floor(Math.random() * gameHeight)
};
function clearContextEnemy() { contextEnemy.clearRect(0, 0, gameWidth, gameHeight) } function Button(xL, xR, yT, yB) {
	this.xLeft = xL;
	this.xRight = xR;
	this.yTop = yT;
	this.yBottom = yB
} Button.prototype.checkClicked = function () { return this.xLeft <= mouseX && mouseX <= this.xRight && this.yTop <= mouseY && mouseY <= this.yBottom };
